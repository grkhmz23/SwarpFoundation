...
const SecurityCard = ({
  ...
  return <SecurityCardInner name={name} email={email} key={animationKey} />;
};
export default SecurityCard;

const SecurityCardInner = ({ name, email }: { name: string; email: string }) => {
  return (
    <div
      className={cn(
        "relative overflow-hidden",
        "flex h-[27rem] w-full max-w-[350px] items-center justify-center",
        "rounded-xl border border-white/10 bg-gray-900",
        "shadow-[0px_0px_0px_1px_rgba(0,255,240,0.05),0px_4px_20px_-4px_rgba(0,0,0,0.5),0px_24px_48px_-12px_rgba(0,0,0,0.4)]",
      )}
    >
      <InfiniteScrambler />
      <ContainerMask />
      <div
        className={cn(
          ...
          "bg-gradient-to-b from-gray-800 to-gray-950 shadow-[0_0_900px_rgba(10,10,10,0.9)]",
        )}
      />
      ...
      <div className="relative rounded-[2px] bg-gray-950/50 px-[3px] py-[3.2px]">
        <div className="relative h-32 w-24 rounded-[2px] bg-gradient-to-br from-gray-700 to-gray-800">
          <FaceCard />
        </div>
      </div>
      <div className="absolute left-0 top-0 h-[200px] w-full [background-image:linear-gradient(to_bottom,rgb(17,24,39)_30%,transparent_100%)]" />
      <div className="absolute left-0 top-4 w-full px-4">
...
const CheckCircle = () => {
  return (
    <div className="relative">
      ...
      <motion.div
        className="absolute left-[5px] top-[5px] flex items-center justify-center text-black"
...
const InfiniteScrambler = () => {
  ...
  return (
    <div className="absolute top-[15%] max-w-[322px]">
      <p className="leading-2 whitespace-normal break-words font-mono text-[13px] text-cyan-500/30">
        {text}
      </p>
    </div>
  );
...
const ContainerMask = () => {
  return (
    <>
      <div className="absolute left-0 top-0 h-full w-[80px] [background-image:linear-gradient(to_right,rgb(17,24,39)_20%,transparent_100%)]" />
      <div className="absolute right-0 top-0 h-full w-[80px] [background-image:linear-gradient(to_left,rgb(17,24,39)_20%,transparent_100%)]" />
    </>
  );
};